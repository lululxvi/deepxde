...
Inverse Problem of Navier Stokes Equation Based on Flow Around Cylinder

training data source：https://doi.org/10.1016/j.jcp.2018.10.045
"""
...
Chat = np.array(
      [np.fromstring(min(re.findall(re.escape("[") + "(.*?)" + re.escape("]"), line), key=len), 
        sep=",",)
        for line in lines])
l, c = Chat.shape
