...
# Load dataset
d = np.load("../examples/operator/antiderivative_aligned_train.npz", allow_pickle=True)
X_train = (d["X"][0].astype(np.float32), d["X"][1].astype(np.float32))
y_train = d["y"].astype(np.float32)
d = np.load("../examples/operator/antiderivative_aligned_test.npz", allow_pickle=True)
