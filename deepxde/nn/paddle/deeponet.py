...
class DeepONet(NN):
    ...
    def merge_branch_trunk(self, x_func, x_loc, index):
        y = paddle.sum(x_func * x_loc, axis=1, keepdim=True)
...
class PODDeepONet(NN):
    ...
    def __init__(
        ...
        self.pod_basis = paddle.to_tensor(pod_basis, dtype=paddle.get_default_dtype())
        if isinstance(activation, dict):
    ...
    def forward(self, inputs):
        ...
        if self.trunk is None:
            # POD only
            x = x_func @ self.pod_basis.T
        else:
            x_loc = self.activation_trunk(self.trunk(x_loc))
            x = x_func @ paddle.concat((self.pod_basis, x_loc), 1).T
            x += self.b
