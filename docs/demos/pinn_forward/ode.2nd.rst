...
Now we deal with the boundary condition :math:`y'(0)=2`.
...
Now we define a function that returns the error of the boundry condition, :math:`y'(0)=2`, which is the difference between the derivative of the output of the network at 0, and 2. The function takes arguments (``inputs``, ``outputs``, ``X``) and outputs a tensor of size ``N x 1``, where ``N`` is the length of ``inputs``. ``inputs`` and ``outputs`` are the network input and output tensors, respectively; ``X`` is the NumPy array of the ``inputs``.
...
.. code-block:: python
    ...
    data = dde.data.TimePDE(geom, ode, [ic, bc], 16, 2, solution=func, num_test=500)
...
Now, we have the PDE problem and the network. We build a ``Model``, choose the optimizer, set the learning rate to 0.001, and train the network for 15000 iterations. We set the weight of the first boundary condition to 0.01, and the weight of the second boundary condition and the PDE residual loss to 1. We also compute the :math:`L^2` relative error as a metric during training.
