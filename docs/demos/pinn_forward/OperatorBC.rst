...
The exact solution is :math:`\frac{50}{81}+\frac{5}{9}t + \frac{31}{81} e^(9t) - 2e^t`.
...
We define a function that returns the error of the first boundry condition, :math:`y(0)=-1`, which is the difference between the output of the network at 0, and -1. The function takes arguments (`inputs`, `outputs`, `X`) and outputs a tensor of size `N x 1`, where `N` is the length of `inputs`. `inputs` and `outputs` are the network input and output tensors, respectively; `X` are the NumPy array of the `inputs`.

.. code-block:: python

    def error_1(inputs, outputs, X):
    	return outputs + 1
...
.. code-block:: python
    ...
    bc1 = dde.icbc.OperatorBC(geom, error_1, boundary_l)

Now we define a function that returns the error of the second boundry condition, :math:`y'(0)=2`, which is the difference between the derivative of the output of the network at 0, and 2. The function should takes the same arguments and should return the same output type as in the previous case.
...
Then, the second boundary condition is defined by

.. code-block:: python

    bc2 = dde.icbc.OperatorBC(geom, error_2, boundary_l)

Now, we have specified the geometry, PDE residual, and boundary conditions. We then define the PDE problem. Note: If you use `X` in `func`, then do not set ``num_test`` when you define ``dde.data.PDE`` or ``dde.data.TimePDE``, otherwise DeepXDE would throw an error. In this case, the training points will be used for testing, and this will not affect the network training and training loss. This is a bug of DeepXDE, which cannot be fixed in an easy way for all backends.

.. code-block:: python

    data = dde.data.TimePDE(geom, ode, [bc1, bc2], 16, 2, solution=func, num_test=100)
