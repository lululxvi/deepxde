...
class DeepONet(NN):
    ...
    def merge_branch_trunk(self, x_func, x_loc):
        y = paddle.einsum("bi,bi->b", x_func, x_loc)
    ...
    def concatenate_outputs(ys):
        return paddle.concat(ys, axis=1)
...
class PODDeepONet(NN):
    ...
    def __init__(
        ...
        super().__init__()
        self.pod_basis = paddle.to_tensor(pod_basis, dtype=config.real(paddle))
        if isinstance(activation, dict):
    ...
    def forward(self, inputs, training=False):
        ...
        if self.trunk is None:
            # POD only
            x = paddle.einsum("bi,ni->bn", x_func, self.pod_basis)
        else:
            x_loc = self.activation_trunk(self.trunk(x_loc))
            x = paddle.einsum(
                "bi,ni->bn", x_func, paddle.concat((self.pod_basis, x_loc), 1)
            )
            x += self.b
